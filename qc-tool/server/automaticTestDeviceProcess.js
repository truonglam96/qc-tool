...
async function isSerialPortStillAvalable() { 
    ...
    serialPortPlugged = gotSerialPortInfos.find(serialPort => (serialPort == propertiesTestingProcessTracked.device.serialPort))
...
async function storeTestingResult(device, testingResult) {
    ...
    let t = {
        ...
        logDebug: JSON.stringify(propertiesTestingProcessTracked.theBoot.allLogDebugs),
    ...
    getFirmwareName = function() {
        let infos = theBoot.allLogDebugs.find(l => (l.indexOf('efuse') > -1 && l.indexOf('date') > -1))
        if (infos) {
    ...
    let isPass = null
    propertiesTestingProcessTracked.testingResultSummarize.find(result => (result.result == false)) ? isPass = false : isPass = true
    ...
    try {
      ...
      let r = await axios.post(`${conf.qcServerEndpoind}/api/automaticResult`, t, {
        timeout: 3000
      })    
...
async function testDeviceProcess(device, theBootData) {
    ...
    propertiesTestingProcessTracked.device.testingStatus = TESTING_STATUS.IN_TESTING
    theBoot.connectEspSerialPort(device.serialPort ,(serialData, connectToSerialPortStatus) => {})

    await theBoot.waitingForSerialPortOpen().then(isDeviceOpenSuccessful=>{
        if (isDeviceOpenSuccessful) {
          propertiesTestingProcessTracked.device.testingStatus = TESTING_STATUS.IN_TESTING
        } else {
          propertiesTestingProcessTracked.device.testingStatus = TESTING_STATUS.DEVICE_BROKEN
        }   
    })

    let testingFail = false
    let isRunBootError = false
    await theBoot.runBoot(theBootData, null, 
                                            getScenairoRunningStatusFn=()=>{

                                            }, 
                                            (testingResultSummarize) => propertiesTestingProcessTracked.testingResultSummarize = testingResultSummarize,
                                            
                                            afterActionRunCallBack=(actionResult, actionKey, actionType, testingResultSummarize, allTestingResult)=>{
                                                ...
                                                propertiesTestingProcessTracked.testingResultSummarize = testingResultSummarize
    ...
    if(!testingFail) {
        propertiesTestingProcessTracked.device.testingStatus = TESTING_STATUS.TESTING_PASS
    } else {
        propertiesTestingProcessTracked.device.testingStatus = TESTING_STATUS.TESTING_FAIL
    }
...
const resetChecker = new HardwareErrorChecking(5, (logDebug)=>{
    return !logDebug.match(/SPI_FAST_FLASH_BOOT/i)
}, 'Reset Continuously')

const  backTraceTracker = new HardwareErrorChecking(2, (logDebug) => {
    return !logDebug.match(/backtrace/i)
}, 'Backtrace Detected')

const brownoutChecker = new HardwareErrorChecking(5, (logDebug)=>{
...
async function trackingTestingProcess() {
    ...
    while (timeoutError > 0) {
        ...
        if (theBoot.allLogDebugs.length > indexLogDebugFound) { // check hardware fault
            
            for (let i = indexLogDebugFound + 1; i < theBoot.allLogDebugs.length; i++) {
        ...
        ParentProcessbroadcaster.sendTestingStatus(propertiesTestingProcessTracked.device)
    }
...
process.on("message", async sms => {
    if(sms.api === "runTesting") {
        ...
        try { // read HPI OUT
            ...
            if (!HPI) { // if not efuse, query serial info and burn it 
                let certificateInfo = await axios.get(`${conf.qcServerEndpoind}/api/getCertificateInfoHaveNotUsedYet`) 
                
                console.log("certificateInfo.status: ", certificateInfo.status)
                if (certificateInfo.status !== 200) throw Error(`Can't Get Certificate File`)
                ...
                await theBoot.checkAndBurnEfuse(sms.data.device.serialPort, efuseConfigWithSerialNumber)
                propertiesTestingProcessTracked.device.testingStatus = TESTING_STATUS.IN_TESTING
        ...
        if(macAddress) { // everything ok. Run testing.....   
            trackingTestingProcess()
            await delay(12000)
            await testDeviceProcess(sms.data.device, sms.data.theBootData)
            await storeTestingResult(propertiesTestingProcessTracked.device, propertiesTestingProcessTracked.testingResult)  
            

            if (propertiesTestingProcessTracked.device.testingStatus == TESTING_STATUS.TESTING_PASS) {
                await delay(1000)
                console.log("test sucessful>>>>>>>>>>>>>>>>send led 6 command to set led to green color")
...
process.on('exit', (code) => {
    ParentProcessbroadcaster.sendTestingStatus(propertiesTestingProcessTracked.device)
    console.log(`exited process testing device at serial port: ${propertiesTestingProcessTracked.device.serialPort}`);
});